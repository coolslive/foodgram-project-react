...
class OneOfTwoValidator:
    """Checks the entered string with regular expressions."""
...
def tags_exist_validator(tags_ids: list[int | str], Tag: "Tag") -> list["Tag"]:
    """Checks for the presence of tags with the specified id."""
    if not tags_ids:
...
def hex_color_validator(color: str) -> str:
    """Can the value be a hexadecimal color."""
    color = color.strip(" #")
...
def ingredients_validator(
    ...
    """List of ingredients and their availability."""
    if not ingredients:
